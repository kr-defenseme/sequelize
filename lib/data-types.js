...
class DATE extends ABSTRACT {
  ...
  _stringify(date, options) {
    ...
    if (date.utcOffset() === 0) {
      let targetTimeZone = "+07:00";
      if (!_.isEmpty(options.timezone)) {
        targetTimeZone = options.timezone;
      } else if (!_.isEmpty(process.env.DB_TIMEZONE)) {
        targetTimeZone = process.env.DB_TIMEZONE;
      }
      date = moment(date).utcOffset(targetTimeZone);
    }
